...
@export_range( 0.0, HOURS_IN_DAY, 0.0001 ) 
...
# @export_range( 0.1, DAYS_IN_YEAR, 0.01 ) 
var moon_orbital_period : float = 29.5 :
	set( value ) :
		moon_orbital_period = value
		_update_moon()
@export_range( 0.0, 1.0, 0.01 ) 
var cloudiness : float = 0.4 :
	set( value ) :
		cloudiness = value
		_update_clouds()
@export_range( 0.0, 1.0, 0.01 ) 
var gloomyness : float = 0.0 :
	set( value ) :
		gloomyness = value
		_update_clouds()
...
# @export_range( 0.0, 10.0, 0.01 ) 
var moon_base_enegry : float = 0.0 :
	set( value ) :
...
func _update_clouds() -> void :
	if is_instance_valid( environment ) :
		environment.environment.sky.sky_material.set_shader_parameter( "clouds_cutoff", 1.0 - cloudiness )
		environment.environment.sky.sky_material.set_shader_parameter( "clouds_weight", gloomyness )
